...
SensorsNode::cycle(std::shared_ptr<NavState> nav_state)
{
  for (auto & group_perceptions : perceptions_) {
    for (auto & p : group_perceptions.second) {
      ...
      // Si los tipos de reloj son distintos (ej: System vs ROS), forzamos la conversión
      // creando un nuevo tiempo con los mismos nanosegundos pero con el reloj del nodo (now).
      if (percept_time.get_clock_type() != now().get_clock_type()) {
